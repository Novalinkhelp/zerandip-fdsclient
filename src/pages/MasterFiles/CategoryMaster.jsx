...
import { fetchCategories } from "../../utils/mockApi";
    ...
    const fetchData = async () => {
      try {
        ...
        const validData = data.filter(
          (cat) => cat.categoryCode && cat.categoryDescription
        );
  ...
  const columns = [
    {
      ...
      render: (item) => (
        <span className="font-medium text-gray-900">{item.categoryCode}</span>
      ),
    },
    {
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          <div className="flex flex-col sm:flex-row items-start sm:items-center">
            <div className="mb-4 sm:mb-0 mr-0 sm:mr-4 md:mr-5 p-2.5 sm:p-3 md:p-3.5 bg-blue-50 rounded-xl text-blue-600 shadow-sm">
              <ListFilter className="h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8" />
            </div>
            <div>
