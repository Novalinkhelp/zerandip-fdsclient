...
const DebtorsLedgerWriteOffCheques = () => {
  const { receipt, cheque } = getFormConfig("writeOffCheques");
  ...
  const columns = [
    {
      ...
      header: "Rem Rec",
      render: (item) => (
        <span className="font-medium text-gray-900">{item.remRec}</span>
      ),
    },
    {
      key: "depositDate",
      header: "Deposit Date",
      render: (item) => (
        <span className="text-gray-800">{item.depositDate}</span>
      ),
    },
    {
      key: "chequeNumber",
    ...
    },
    {
      ...
      render: (item) => (
        <span className="text-gray-600">{item.receiptNumber}</span>
      ),
    },
    {
      key: "repCode",
    ...
    },
    {
      ...
      render: (item) => (
        <span className="text-gray-600">{item.chequeAmount}</span>
      ),
    },
  ];
    ...
    <div className="space-y-8">
      <div className="flex flex-col mb-4">
        <h1 className="text-2xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-1 sm:mb-1.5">
          Write Off POD / RTN Cheques
        </h1>
        <p className="text-gray-600 text-base sm:text-lg">
          Handle writing off pod / rtn cheques in debtors ledger.
        </p>
      </div>
...
export default DebtorsLedgerWriteOffCheques;
